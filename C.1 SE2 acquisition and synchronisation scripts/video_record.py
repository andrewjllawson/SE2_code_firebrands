...
Emberometer Video, Thermal, and Temperature Acquisition
-------------------------------------------------------

This module is launched by the top-level controller (run.py) to perform
timestamped multi-sensor recording of:
  ...
  - Local ambient temperature from a MAX31855 thermocouple interface (via SPI)

Each recording session:

  - Creates a new directory named using the GNSS-derived recording start time
  - Stores:
      ...
      * GNSS start time, system time, position, and device ID (GNSS_adjustment.txt)
      * Temperature log from the thermocouple interface (temperature_log.csv)
      * A simple recording_complete.txt flag
      * A session copy of the device specification file

The controller (run.py) is responsible for:

  - Ensuring GNSS lock before starting this process
  - Passing GPS_RECORDING_START_UTC, GPS_RECORDING_START_LAT, and
    GPS_RECORDING_START_LON in the environment
...
class MAX31855:
    ...
    def read_raw(self):
        """
        Read raw SPI data from the thermocouple interface.
    
        Returns:
            tuple[int, list[int]]:
                - 32-bit integer value representing the decoded SPI word
                - raw byte list returned by the SPI transfer
    ...
    def read_celsius(self):
        def read_celsius(self):
            ...
            The 32-bit SPI word contains:
              - a fault indicator bit
              - signed thermocouple temperature data
              - raw diagnostic information preserved here for logging/debugging
        
            Returns:
                tuple[float, list[int]]:
                    - temperature in °C
                    - raw byte list from the sensor read
        
            Raises:
                RuntimeError: if a thermocouple fault is detected.
            """
        value, raw = self.read_raw()
...
def temp_logging_thread(session_dir, rgb_start_time, stop_event):
    """
    Periodically log ambient temperature from the thermocouple interface to CSV.

    Samples once per second (TEMP_LOG_INTERVAL) until stop_event is set, and
    writes:

        time_s, temperature_C, bit

    into temperature_log.csv located inside the recording session directory.

    Here:
      - time_s is the elapsed time relative to the RGB start reference
      - temperature_C is the measured thermocouple temperature
      - bit stores the raw sensor bytes for debugging/diagnostic purposes

    Args:
        session_dir (str): Path to the recording session directory.
        rgb_start_time (float): Unix timestamp used as the relative time reference.
        stop_event (threading.Event): Cooperative stop flag for clean exit.
    """
    csv_path = os.path.join(session_dir, "temperature_log.csv")
    sensor = None

    # Try initialising the thermocouple interface; if that fails, still create the CSV header and exit.
    try:
        ...
        with open(csv_path, mode='w', newline='') as csvfile:
            ...
            writer.writerow(["time_s", "temperature_C", "bit"])
        return
    ...
    with open(csv_path, mode='w', newline='') as csvfile:
        ...
        try:
            while not stop_event.is_set():
                ...
                try:
                    temp_c, bit = sensor.read_celsius()
                ...
                # Write relative time, Celsius temperature, and raw diagnostic bytes
                writer.writerow([f"{t_rel:.3f}", temp_c, bit])
                csvfile.flush()
...
def main():
    ...
    Responsibilities:
      ...
      - Save GNSS start time, system time, position, and device ID
      - Copy the session device specification file
      - Start thermal, RGB, thermocouple, and CSV logging threads
      - Periodically compute and print approximate FPS
      - On termination:
          * Stop all threads
    ...
    # -------------------------
    # Persist GNSS start time, system time, and location metadata
    # -------------------------  
    ...
    system_time = datetime.now().strftime("D%Y%m%d_T%H%M%S")
